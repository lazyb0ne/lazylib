...
from utils.my_db import db_urls
...
def get_link_by_url(base_url, deep):
    ...
    if s and len(s) > 100:
        ...
        query = {'url': base_url}
        # 执行更新操作
        db_urls().update_one(query, update_data)
    # 遍历内部链接数组
    for link in internal_links:
        # 补充为完整的URL
        url = urljoin(base_url, link)
        # 判断URL是否已存在
        if db_urls().find_one({'url': url,'kind': 'in_pc'}):
            print(f"URL '{deep} {url}' already exists in the database.")
        else:
            # 插入URL到数据库
            db_urls().insert_one({'url': url, 'from_url': base_url, 'deep': deep, 'kind': 'in_pc'})
    ...
    for url in external_links:
        ...
        if db_urls().find_one({'url': url, 'kind': 'in_pc'}):
            ...
            # 插入URL到数据库
            db_urls().insert_one({'url': url, 'from_url': base_url, 'deep': deep, 'kind': 'out_pc'})
            print(f"URL '{deep} {url}' inserted into the database.")
    ...
    for link in internal_links_mobile:
        ...
        url = urljoin(base_url, link)
        # 判断URL是否已存在
        if db_urls().find_one({'url': url, 'kind': 'in_mobile'}):
            print(f"URL '{deep} {url}' already exists in the database.")
        else:
            # 插入URL到数据库
            db_urls().insert_one({'url': url, 'from_url': base_url, 'deep': deep, 'kind': 'in_mobile'})
            print(f"URL '{deep} {url}' inserted into the database.")
    # 遍历外部链接数组
    for url in external_links_mobile:
        ...
        if db_urls().find_one({'url': url, 'kind': 'in_mobile'}):
            ...
            # 插入URL到数据库
            db_urls().insert_one({'url': url, 'from_url': base_url, 'deep': deep, 'kind': 'out_mobile'})
            print(f"URL '{deep} {url}' inserted into the database.")
...
def start_by_deep(deep):
    ...
    for url in urls:
        ...
        if len(results_list) > 0:
            ...
            # 执行更新操作
            try:
                db_urls().update_many(query, update)
            except Exception as e:
                time.sleep(10);
                try:
                    db_urls().update_many(query, update)
