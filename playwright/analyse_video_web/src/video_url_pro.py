...
from utils.my_url_util import get_page_source_with_playwright, get_source_with_playwright_by_mobile, \
    get_source_with_iframe, get_page_source_with_iframes, is_valid_url, base_url_with_http
...
def download_website_text_and_links(url, is_phone=False, headless=True):
    ...
    for link in links:
        href = link.get('href')
        if href.startswith('http'):  # 判断链接是否以 http 开头，即为绝对路径
            if base_url in href:  # 如果链接中包含基准 URL，即为内链
    ...
    print(internal_links)
    return internal_links, external_links, source


def get_link_by_url(base_url, deep):
    try:
        internal_links, external_links, source = download_website_text_and_links(base_url, True)
        return internal_links, external_links
    except Exception as e:
        print(f"An exception occurred: get_link_by_url {str(e)}")
        return
    # 设置要更新的字段和值
    if source and len(source) > 100:
        update_data = {
            '$set': {
                'source_mobile': source,
...
def update_urls_for_iframes(url, page_source, iframe_src_list):
    ...
    # 根据URL更新数据
    db().update_one({"url": url.replace("www.","")}, {"$set": data}, upsert=True)
